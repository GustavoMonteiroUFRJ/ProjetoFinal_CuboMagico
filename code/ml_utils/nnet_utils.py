...
def addNnetArgs(parser):
    ...
    parser.add_argument('--num_f_h', type=int, default=500, help="Number of frist hidden neurons") #GUSTAVO
...
def model_fn(features,labels,mode,args):
    ...
    # dropoutSonnet = lambda x:   tf.nn.dropout(x,keep_prob=1-args.drop_p) #GUSTAVO: old 
    dropoutSonnet = lambda x:   tf.nn.dropout(x,rate=args.drop_p)
    ...
    nnet_layers.append(lambda x: layers.dense(x, args.num_f_h, args.act_type, isTraining, doBatchNorm, args.l2, weightNorm, layerNorm, angleNorm))
